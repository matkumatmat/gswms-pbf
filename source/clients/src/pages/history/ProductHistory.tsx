...
export default function ProductHistory() {
  ...
  const [filterBy, setFilterBy] = useState('batch'); // batch, kodeBarang, namaKonsumen, field
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await execute(() => api.call('getProductHistory', { 
        filterBy, 
        value, 
        fieldName, 
        startDate, 
  ...
  const columns = [
    ...
    { header: 'Type', accessor: 'jenisTransaksi' as const },
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <h2 className="text-xl font-bold">Product History</h2>
