...
export default function ShippingEmbalageHistory() {
  const { api, execute, loading } = useApi();
  const [data, setData] = useState([]);
  
  const [filterBy, setFilterBy] = useState('noDokumen'); // noDokumen, kodeBarang, namaBarang, field, year
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await execute(() => api.call('getShippingEmbalageHistory', { 
        filterBy, 
        value, 
        fieldName, 
        startDate, 
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <h2 className="text-xl font-bold">Shipping Embalage History</h2>
