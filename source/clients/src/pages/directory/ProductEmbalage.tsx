...
export default function ProductEmbalageDirPage() {
  ...
  const loadData = async (currentPage: number) => {
    try {
      let res;
      if (searchValue.trim() !== '') {
        res = await api.call('getMasterProductEmbalagesByField', { field: searchField, value: searchValue });
        ...
        res = await api.call('getMasterProductEmbalagesPaginated', { page: currentPage, limit: 10, statues: 'ACTIVE' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await execute(() => api.call('deleteMasterProductEmbalage', { id }));
      loadData(page);
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await execute(() => api.call('updateMasterProductEmbalage', { id: editingId, data: formData }));
      } else {
        // CREATE: kirim LANGSUNG formData
        await execute(() => api.call('createMasterProductEmbalage', formData));
      }
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold">Product Embalage Master</h2>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" className="w-full border p-2 rounded" value={formData.batch || ''} onChange={e => setFormData({...formData, batch: e.target.value})} />
            </div>
            <div>
