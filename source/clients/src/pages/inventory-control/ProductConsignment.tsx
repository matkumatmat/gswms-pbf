...
export default function ProductConsignment() {
  ...
  const loadData = async (currentPage: number, selectedYear: string) => {
    try {
      const res = await api.call('getTransactional', { year: selectedYear, sheet: 'ALL_CONS', page: currentPage, limit: 10 });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await execute(() => api.call('deleteTransactional', { id, year, sheet: 'ALL_CONS' }));
      loadData(page, year);
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await execute(() => api.call('updateTransactional', { id: editingId, year, sheet: 'ALL_CONS', data: formData }));
      } else {
        await execute(() => api.call('createTransactional', { year, sheet: 'ALL_CONS', data: formData }));
      }
  ...
  const columns = [
    ...
    { header: 'Saldo Awal', accessor: 'saldoAwal' as const },
    { header: 'Pemakaian', accessor: 'pemakaian' as const },
    { header: 'FTI', accessor: 'fti' as const },
    { 
      header: 'Actions', 
      accessor: (row: any) => (
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <h2 className="text-xl font-bold">Product Consignment (ALL_CONS)</h2>
        <div className="flex items-center gap-4">
          <select className="border p-2 rounded" value={year} onChange={e => { setYear(e.target.value); setPage(1); }}>
            <option value="2025">2025</option>
            <option value="2026">2026</option>
          </select>
          <Button onClick={handleCreate}>Add Consignment</Button>
        </div>
      </div>
      
      {/* Debug box (opsional, bisa dihapus nanti) */}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Consignment' : 'Add Consignment'}>
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" className="w-full border p-2 rounded" value={formData.saldoAwal || ''} onChange={e => setFormData({ ...formData, saldoAwal: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Pengiriman</label>
              <input type="number" className="w-full border p-2 rounded" value={formData.pengiriman || ''} onChange={e => setFormData({ ...formData, pengiriman: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Pemakaian</label>
              <input type="number" className="w-full border p-2 rounded" value={formData.pemakaian || ''} onChange={e => setFormData({ ...formData, pemakaian: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Pengembalian</label>
              <input type="number" className="w-full border p-2 rounded" value={formData.pengembalian || ''} onChange={e => setFormData({ ...formData, pengembalian: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">FTI</label>
              <input type="number" className="w-full border p-2 rounded" value={formData.fti || ''} onChange={e => setFormData({ ...formData, fti: e.target.value })} />
            </div>
          </div>
