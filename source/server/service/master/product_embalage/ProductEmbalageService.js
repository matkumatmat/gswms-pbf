// source/server/service/productEmbalage/ProductEmbalageMasterService.js

function ProductEmbalageMasterService(repository) {
  this.repo = repository;
  this.currentUser = null;

  var masterCfg = ApplicationConfig.dataSources.master.productEmbalage;
  var pembCfg = masterCfg.configs.find(function(c) { return c.type === 'PEMB'; });
  if (!pembCfg) throw new Error('PEMB config not found');

  var cacheGroup = CacheManager.resolveCacheGroup({
    domain: 'master',
    spreadsheetId: masterCfg.spreadsheetId,
    sheetName: pembCfg.sheetName
  });
  ...
  this._filterByStatues = function (records, statues) {
    if (!statues) return records.filter(function(r) { return (r.statues || '').toUpperCase() !== 'DELETED'; });
    ...
    return records.filter(function(r) { return (r.statues || '').toUpperCase() === statues.toUpperCase(); });
  };
