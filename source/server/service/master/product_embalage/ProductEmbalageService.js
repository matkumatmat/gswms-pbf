...
function ProductEmbalageMasterService(repository) {
  ...
  this._filterByStatues = function (records, statues) {
    if (!statues) return records.filter(r => (r.statues || '').toUpperCase() !== 'DELETED');
  ...
  this.getAll = (statues) => this._filterByStatues(this.repo.getAll(), statues);
  this.getById = (id) => this.repo.findById(id);

  this.getPaginated = (page, limit, statues) => {
  ...
  this.findByField = (field, value) => this.repo.findByField(field, value);
