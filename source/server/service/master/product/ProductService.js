...
function ProductMasterService(repository) {
  ...
  var productCfg = masterCfg.configs.find(function(c) { return c.type === 'PRODUCT'; });
  if (!productCfg) throw new Error('PRODUCT config not found');

  var cacheGroup = CacheManager.resolveCacheGroup({
    domain: 'master',
    spreadsheetId: masterCfg.spreadsheetId,
    sheetName: productCfg.sheetName
  });
  ...
  this._filterByStatues = function (records, statues) {
    if (!statues) return records.filter(function(r) { return r.statues !== 'DELETED'; });
  ...
  this.getAll = function (statues) {
    ...
    if (statues) return this._filterByStatues(all, statues);
    return all.filter(function(r) { return r.statues !== 'DELETED'; });
  };
